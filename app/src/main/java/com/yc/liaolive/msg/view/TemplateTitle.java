...
public class TemplateTitle extends RelativeLayout {
    ...
    private int mTitleStyle;//默认是白底黑字


    public TemplateTitle(Context context, AttributeSet attrs) {
        super(context, attrs);
        LayoutInflater.from(context).inflate(R.layout.view_chat_title, this);
        ...
        try {
            ...
            mTitleStyle = ta.getInt(R.styleable.CommentTitleView_commentTitleStyle, 0);
            setUpView();
        } finally {
            ta.recycle();
        }
    ...
    public void setMoreImgColor(int color) {
        ImageView moreImgView = (ImageView) findViewById(R.id.img_more);
        moreImgView.setColorFilter(color);
    }

    public void setMoreImgVisable(int visable) {
        ImageView moreImgView = (ImageView) findViewById(R.id.img_more);
        moreImgView.setVisibility(visable);
    }

    public void setBackImgColor(int color) {
        ImageView backImgView = (ImageView) findViewById(R.id.img_back);
        backImgView.setColorFilter(color);
    }
