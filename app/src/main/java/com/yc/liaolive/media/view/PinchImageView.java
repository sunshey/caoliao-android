...
public class PinchImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public void setOnDoubleClickListener(OnDoubleClickListener onDoubleClick) {
    ...
    private GestureDetector mGestureDetector = new GestureDetector(PinchImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (mPinchMode == PINCH_MODE_SCROLL && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                if(null!=onDoubleClick) onDoubleClick.onDoubleClick();
    ...
    public interface OnTouchEventListsner{
        void onTouchEvent(MotionEvent event);
    }

    private OnTouchEventListsner mOnTouchEventListsner;

    public void setOnTouchEventListsner(OnTouchEventListsner onTouchEventListsner) {
        mOnTouchEventListsner = onTouchEventListsner;
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        super.onTouchEvent(event);
        //关心触摸事件的可以在不影响此控件触摸分发事件的情况下注册
        if(null!=mOnTouchEventListsner) mOnTouchEventListsner.onTouchEvent(event);
