...
import com.yc.liaolive.media.bean.VideoLiveTagInfo;
...
public class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        //color
        mTextColor = typedArray.getColor(R.styleable.FlowLayout_flowTextColor, CommonUtils.getColor(R.color.white));
        typedArray.recycle();
...
//        Log.i("test", "sizeHeight" + sizeHeight);
     ...
     * 设置数据
     */
    public void setListData(List<VideoLiveTagInfo> list) {
        int count = list.size();
        tagsSize=count;
        for (int i = 0; i < count; i++) {
            TextView tv;
            if(isColorful){
                tv = (TextView) mInflater.inflate(R.layout.flowlayout_textview, this, false);
                Random random = new Random();
                int ranColor = 0xff000000 | random.nextInt(0x00ffffff);
                tv.setBackgroundColor(ranColor);
                tv.setTextColor(mTextColor);
            }else{
                tv = (TextView) mInflater.inflate(R.layout.flowlayout_textview_bg, this, false);
            }
            tv.setText(list.get(i).getTitle());
            tv.setTag(list.get(i).getId());
            tv.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (onTagClickListener != null)
                        onTagClickListener.TagClick((String) v.getTag());
                }
            });
            this.addView(tv);
        }
     ...
     * @param data
     */
    public void addTag(VideoLiveTagInfo data) {
        tagsSize++;
        TextView tv;
        if(isColorful){
            tv = (TextView) mInflater.inflate(R.layout.flowlayout_textview, this, false);
