...
public class SideBar extends View {
    ...
    public SideBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        setBackgroundResource(R.drawable.shape_quick_bar_bg);
    }
    ...
    private Runnable dialogRunnable = new Runnable() {
        @Override
        public void run() {
            if(null!=mTextDialog&&mTextDialog.getVisibility()!=View.INVISIBLE){
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_UP:
                ...
                invalidate();
                if (mTextDialog != null) {
                    SideBar.this.postDelayed(dialogRunnable,300);
                }
                break;
            default:
                if (oldChoose != c) {
