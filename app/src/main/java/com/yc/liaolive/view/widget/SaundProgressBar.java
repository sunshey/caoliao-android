 ...
 * 一个增强版本的进度提供了更大的控制权
 *如何绘制和显示进度条。动机是允许我们
 *更自由地定制进度条的外观。我们现在可以显示
 *进度栏末端的圆形帽，并可选显示叠加。
 *我们还可以提出进度指标，显示完成的百分比或
 *一个更加个性化的价值通过执行格式化程序界面
 * @author kince
 * 
 */
public class SaundProgressBar extends ProgressBar {
	...
	public SaundProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		if (a != null) {
			m_textPaint.setTextSize(a.getDimension(
					R.styleable.SaundProgressBar_textSizeS, 10));
	...
	/**
	 * 而不是使用裁剪区域发现进度条的进度增加我们增加drawable区域进度条和图案叠加。这样做使我们拥有更大的控制权，并允许我们显示进度条上的圆形盖子。
	 */
