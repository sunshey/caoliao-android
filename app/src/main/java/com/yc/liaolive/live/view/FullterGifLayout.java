...
public class FullterGifLayout extends RelativeLayout {
    ...
    private long delayedTime =1000;//默认隔多少秒自动切换
    ...
    private int[] heartRes = new int[]{
            ...
            R.drawable.resource_heart9,R.drawable.resource_heart0
    };
    ...
    public FullterGifLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    public FullterGifLayout(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    public void setAutoaddHeartDurtion(long delayedTime){
        this.delayedTime =delayedTime;
    }
    /**
     * 自动切换显示文字
     */
    public void startAddHeart(){
        if(null!=sHandler&&null!=goNextPageRunnable){
            stopAddHeart();
            sHandler.postDelayed(goNextPageRunnable, delayedTime);
        }
    }
    ...
    private Runnable goNextPageRunnable = new Runnable() {
        ...
        public void run() {
            addHeart();
            sHandler.postDelayed(this, delayedTime);
        }
    };
    ...
    public void onDestroy(){
        stopAddHeart();
        if(null!=sHandler) sHandler.removeMessages(0);
        sHandler=null;goNextPageRunnable=null;
    }
}
