...
public class DrawBigMulitAnimationView extends RelativeLayout{
    ...
    public DrawBigMulitAnimationView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public void start(final int powerNum){
        ...
        if(isRunning){
            ...
            //480毫秒过后绘制中奖倍数
            SpannableStringBuilder stringBuilder = LiveChatUserGradleSpan.drawNumFromat(String.valueOf(powerNum));
            //拼接自定义倍数Image
            stringBuilder.append(LiveChatUserGradleSpan.drawPowerFromat("倍"));
        ...
        isRunning=true;
        //动画播放
        mSurfaceView.setImageResource(R.drawable.draw_big_anim);
    ...
    private void startTask(){
        ...
        mTask = new TimerTask() {
            @Override
            public void run() {
                DrawBigMulitAnimationView.this.post(new Runnable() {
                    @Override
                    public void run() {
                        Context context = getContext();
                        if(null!=context){
                            GoldWireLayout goldWireLayout = new GoldWireLayout(getContext());
                            int startX = Utils.getRandomNum(ScreenUtils.dpToPxInt(72f), ScreenUtils.dpToPxInt(92f));
                            int startY=(mStartPosition[1]+ScreenUtils.dpToPxInt(30f));
                            ...
                            int[] awardEndLocation = GiftHelpManager.getInstance().getAwardEndLocation();
                            goldWireLayout.setEndPosition(new Point(awardEndLocation[0]+ScreenUtils.dpToPxInt(20f), awardEndLocation[1]+ScreenUtils.dpToPxInt(15f)));
