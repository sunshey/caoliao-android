...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    public void update(List<?> imageUrls) {
        if(null!=mImageUrls) mImageUrls.clear();
        if(null!=imageViews) imageViews.clear();
        if(null!=indicatorImages) indicatorImages.clear();
        if(null==mImageUrls) mImageUrls =new ArrayList<>();
    ...
    private void setTitleStyleUI() {
        if (titles.size() != mImageUrls.size()) {
    ...
    private void setImageList(List<?> imagesUrl) {
        if (imagesUrl == null || imagesUrl.size() <= 0) {
            ...
            if(null!=bannerDefaultImage) bannerDefaultImage.setVisibility(GONE);
            return;
        }
    ...
    /**
     * 修复一个BUG，当数据需要更新时，不能重复调用setAdapter ,，应当调用notifyDataSetChanged
     * 避免 IllegalStateException 异常
     */
