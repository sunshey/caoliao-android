...
public class FURenderer implements OnFUControlListener {
    ...
    public Handler getmFuItemHandler() {
    ...
    public void onSurfaceCreated() {
        ...
        if (isNeedFaceBeauty) {
            getmFuItemHandler().sendEmptyMessage(FUItemHandler.HANDLE_CREATE_BEAUTY_ITEM);
        }

        if (isNeedAnimoji3D) {
            getmFuItemHandler().sendEmptyMessage(FUItemHandler.HANDLE_CREATE_ANIMOJI3D_ITEM);
        }
    ...
    public boolean isFRONT() {
        return mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_FRONT;
    }
...
//          faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "blur_blend_ratio", 1);
    ...
    @Override
    public void onFaceStylelSelected(float progress) {
        isNeedUpdateFaceBeauty = true;
        mFaceShapeLevel = progress;
    }
    ...
    @Override
    public int onRenderingFrame(int textureId, int width, int height, long timestampNs) {
        Log.e("onRenderingFrame", "onRenderingFrame textureId " + textureId + " width= " + width + " height= " + height + " timestampNs=" + timestampNs);
        ...
        if (isFRONT()) {
            matrix[5] = -1.0f;
            fuTuxId = onDrawFrame(fuTuxId, width, height, matrix);
        }
        fuTuxId = onDrawFrame(frameData, fuTuxId, width, height);
        fuTuxId = onDrawFrame(fuTuxId, width, height, matrix);
        for (int i = 0; i < 10; i++) {
            if (enabled[i] == 1) {
    ...
    public void onPreviewFrame(byte[] data, int width, int height, int rotation, int fmt, long timestampNs) {
        frameData = data;
        Log.e("onPreviewFrame", "data.length=" + data.length + "--width=" + width + "--height=" +
...
//        }
    ...
    /**
     * 照相机发生了变化
     * @param currentCameraType 当前摄像头ID
    ...
    public void onCameraChange(boolean isFrontCamera) {
        ...
        onCameraChange(cameraId, orientation);
    }
    ...
    public static class Builder {
        ...
        private int inputTextureType = faceunity.FU_ADM_FLAG_ENABLE_READBACK;
    ...
    public void destroyItems() {
        ...
        deleteFBO();
        frameData=null;
        if(null!=mFuItemHandler){
            mFuItemHandler.removeMessages(FUItemHandler.HANDLE_CREATE_ITEM);
            mFuItemHandler.removeMessages(FUItemHandler.HANDLE_CREATE_BEAUTY_ITEM);
            mFuItemHandler.removeMessages(FUItemHandler.HANDLE_CREATE_ANIMOJI3D_ITEM);
        }
        landmarksData=null;expressionData=null; rotationData=null;pupilPosData=null;rotationModeData=null;
        if(null!=mEventQueue) mEventQueue.clear();
        mEventQueue=null;mFuItemHandler=null;
    }
